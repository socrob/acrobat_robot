    ...
    def receive_loop():
        ...
        while 1:
            ...
            if msg[0] == 'pushys':
                p.pushys(msg[1])
                counter+=1
                print("I heard it {}".format(counter))
            else:
                return
...
class proxy_enabler(object):
    def __init__(self,lines):
        self.ip = interprocess_plotter(2)
        
    def desenha(self,lista1,lista2):
        self.ip.pushys([numpy.sin(lista1),numpy.sin(lista2)])

if __name__ == "__main__":
    ip = proxy_enabler(2)
    # for example purposes we will access the daemon and name server ourselves and not use serveSimple
    with Pyro4.Daemon() as daemon:
        ip_uri = daemon.register(ip)
        with Pyro4.locateNS() as ns:
            ns.register("proxy.enabler", ip_uri)
        print("Plotter Ready Sir.")
        daemon.requestLoop()