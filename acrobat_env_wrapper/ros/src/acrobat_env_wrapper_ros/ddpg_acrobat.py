...
for ep in range(num_episodes):
	...
	for t in range(max_steps_ep):
		input_observation = np.reshape(observation,(1,len(observation)))
		# choose action based on deterministic policy
		action_for_state, = sess.run(actions, 
			feed_dict = {state_ph: input_observation, is_training_ph: False})
		#action_for_state = action_for_state[0]
		...
		action_for_state = clamper(action_for_state)
		#action_for_state = action_for_state[0]
		# take step
		next_observation, reward, done = env.step(action_for_state)
		#if ep%10 == 0: env.render()
		if t == max_steps_ep: done = True
		total_reward += reward
